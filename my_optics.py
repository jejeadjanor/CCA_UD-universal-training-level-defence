...
class OPTICS(ClusterMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if self.cluster_method == "xi":
            labels_, clusters_, sdas, suas = cluster_optics_xi(
...
def compute_optics_graph(
    ...
    for ordering_idx in range(X.shape[0]):
        ...
        point = index[np.argmin(reachability_[index].astype(np.float16))]
        # point_list = np.where(reachability_==np.min(reachability_[index]))
        #
        # point = point_list[0][0]
        # if point in [78, 227, 1028]:
        #     print('here')
...
def cluster_optics_xi(
    ...
    # get the steepup and steepdown area
    sdas, suas = _xi_steep_areas(
        reachability[ordering],
        predecessor[ordering],
        ordering,
        xi,
    ...
    return labels, clusters, sdas, suas
    ...
    for steep_index in iter(np.flatnonzero(steep_upward | steep_downward)):
        ...
        # steep upward areas
            ...
            for D in sdas:
                ...
                # if c_end - c_start + 1 < min_cluster_size: # original condition
                if U_start - D["end"] + 1 < min_cluster_size: # my condition
                    continue

                # Definition 11: criterion 1
                if c_start > D["end"]:  # c_start and D["end"] are index not reachability value
                    continue
...
def _xi_steep_areas(
    ...
    xi_complement = 1 - xi
    sdas, suas = [], []  # steep down areas, introduced in section 4.3.2 of the paper
    clusters = []
    index = 0
    mib = 0.0  # maximum in between, section 4.3.2
